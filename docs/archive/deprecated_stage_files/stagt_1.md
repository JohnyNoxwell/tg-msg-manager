...
Этап опирается на уже выполненные рефакторинги из `backlog/archive/TODO.md` и не должен заново выполнять то, что уже выделено в helpers, typed DTO и event abstractions.
...
- не повторяем уже завершённые экстракции из `backlog/archive/TODO.md`;
...
- `backlog/archive/TODO.md` считается фильтром против повторного выполнения уже закрытых refactor blocks;
...
- не повторены уже закрытые refactors из `backlog/archive/TODO.md`;
