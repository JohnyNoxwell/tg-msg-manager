# Pre-package internal changelog 4.x/3.x

This archive preserves internal/stage-numbered history that previously lived in the root `CHANGELOG.md`.
These entries are historical project notes and are not public package releases for `tg-msg-manager`.
Use the root `CHANGELOG.md` for current package semantic-version release history.
